...
<head>
    ...
    <script>
        function modifyProduct() {
            ...
            // Abrir el formulario de modificación con los datos del producto
            var urlForm = "formulario_productos_v2.html";
            window.open(urlForm + "?id=" + id + "&nombre=" + nombre + "&marca=" + marca +
                        "&modelo=" + modelo + "&precio=" + precio + "&unidades=" + unidades +
                        "&detalles=" + detalles, "_blank");
        }
    </script>
</head>
<body>
        ...
        <table class="table table-striped">
            ...
            <tbody>
                <?php foreach ($productos as $producto) : ?>
                    <tr id="producto-<?= htmlspecialchars($producto['id']) ?>">
                        <td class="row-data"><?= htmlspecialchars($producto['id']) ?></td>
                        ...
                        <td class="row-data"><?= htmlspecialchars(utf8_encode($producto['detalles'])) ?></td>
                        <td><img src="<?= htmlspecialchars($producto['imagen']) ?>" alt="Imagen de <?= htmlspecialchars($producto['nombre']) ?>" style="width:100px;" /></td>
                        <td><input type="button" value="Modificar" onclick="modifyProduct()" /></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
